"""Regression test for trading._order_side() -- the fix for a real production bug where every
...
from app.trading import _order_side
...
def test_scanner_pe_signal_is_buy_not_sell():
    ...
    assert _order_side(signal, evaluation) == "buy"
...
def test_explicit_sell_action_is_respected():
    ...
    assert _order_side(signal, evaluation) == "sell"
...
def test_equity_signal_with_no_action_falls_back_to_direction():
    ...
    assert _order_side(signal, {"direction": "bearish"}) == "sell"
    assert _order_side(signal, {"direction": "bullish"}) == "buy"
